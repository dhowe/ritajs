...
describe("Markov", function() {
  ...
  it("should call RiMarkov", function() {
    ...
    eq(rm.size(), 0);
    expect(function() {
      let rm2 = new RiMarkov(3, { maxLengthMatch: 2 });
    }).to.throw();
  });
  it("should call RiTa.markov", function() {
    ...
    expect(function() {
      rm = RiTa.markov(3, { maxLengthMatch: 2 });
    }).to.throw();
  });
  ...
  function markovEquals(rm, copy) {
    Object.keys(rm).filter((k) => !/(root|input|sentenceStarts|sentenceEnds|leaves)/.test(k)).forEach((k) => expect(rm[k], "failed on " + k).eq(copy[k]));
